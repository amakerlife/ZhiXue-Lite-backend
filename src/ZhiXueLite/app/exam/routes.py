...
def zhixue_account_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.zhixue_username or not current_user.zhixue_password:
...
def get_exam_list():
    ...
    从数据库获取当前学生的考试列表
    """
    exams = UserExam.query.filter_by(zhixue_username=current_user.zhixue_username).all()
...
def fetch_exam_list():
    ...
    student_account = login_student(
        current_user.zhixue_username, current_user.zhixue_password
    )
    ...
    for exam in exams:
        # 检查考试是否已存在
        existing_exam = Exam.query.filter_by(exam_id=exam.id).first()
        if not existing_exam:
            new_exam = Exam()
            new_exam.exam_id = exam.id
            new_exam.exam_name = exam.name
            db.session.add(new_exam)
            db.session.commit()

        # 检查用户考试记录是否已存在
        user_exam = UserExam.query.filter_by(
            zhixue_username=current_user.zhixue_username, exam_id=exam.id
        ).first()
        if not user_exam:
            new_user_exam = UserExam()
            new_user_exam.zhixue_username = current_user.zhixue_username
            new_user_exam.exam_id = exam.id
            db.session.add(new_user_exam)
            db.session.commit()

    result = UserExam.query.filter_by(zhixue_username=current_user.zhixue_username).all()
    exams = [
        {
            "id": exam.exam_id,
